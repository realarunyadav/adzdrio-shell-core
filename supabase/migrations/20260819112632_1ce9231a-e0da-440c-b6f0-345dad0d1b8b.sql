-- Phase 3: Employee Foundation (Corrected Role Cases)
...
-- Policy: Admins can view/update records in their organization
DO $$ BEGIN
...
-- Policy: Manager/subordinate visibility
DO $$ BEGIN
...
-- Policy: Staff can view peers in same business
DO $$ BEGIN
...
CREATE INDEX IF NOT EXISTS idx_employees_reports_to_id ON public.employees(reports_to_id);

-- 6. Trigger for updated_at
CREATE OR REPLACE FUNCTION public.handle_employee_updated_at()
RETURNS TRIGGER AS $$
BEGIN
    NEW.updated_at = now();
    RETURN NEW;
...
DO $$ BEGIN
    CREATE TRIGGER on_employee_updated
        ...
        FOR EACH ROW
        EXECUTE PROCEDURE public.handle_employee_updated_at();
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
