...
function LeadQueuesPage() {
  ...
  const startEdit = (queue: LeadQueue) => {
    ...
    let editValue = "";
    if (conditions.minScore !== undefined) { editField = "scoreMin"; editValue = String(conditions.minScore); }
    else if (conditions.maxScore !== undefined) { editField = "scoreMax"; editValue = String(conditions.maxScore); }
    else {
      const match = ["source", "stage", "city", "state"].find((key) => conditions[key] !== undefined);
      if (match) { editField = match as QueueRule["field"]; editValue = String(conditions[match]); }
    }
    setEditing(queue);
    setName(queue.name);
    setActive(queue.isActive);
    setField(editField);
    setOperator(editField.startsWith("score") ? (editField === "scoreMin" ? "gte" : "lte") : "equals");
    setValue(editValue);
    setStrategy((config.assignmentType ?? "ROUND_ROBIN") as Strategy);
    setSelectedUsers(Array.isArray(config.userIds) ? config.userIds : []);
    setOpen(true);
  };
  ...
  const save = async () => {
    ...
    else conditions[field] = operator === "contains" ? value.trim() : value.trim();
  ...
  const toggle = async (queue: LeadQueue) => {
    try {
      await leadQueueService.update(queue.id, { isActive: !queue.isActive });
      await load(true);
  ...
  const remove = async (queue: LeadQueue) => {
    ...
    try {
      await leadQueueService.remove(queue.id);
  ...
  return <div className="flex flex-col gap-6 p-6 animate-in fade-in duration-500">
    <PageHeader
      eyebrow="Adzdrio CRM"
      title="Lead Routing Queues"
      description="Configure organization-scoped matching rules, active salespeople, and automatic assignment strategy."
    ...
    <SectionCard title="Routing configuration" description="Each active queue is evaluated in creation order. The first matching queue selects its active salespeople and assignment strategy." contentClassName="p-0">
      <Table><TableHeader><TableRow><TableHead>Queue</TableHead><TableHead>Status</TableHead><TableHead>Match rule</TableHead><TableHead>Strategy</TableHead><TableHead>Salespeople</TableHead><TableHead>Updated</TableHead><TableHead className="text-right">Actions</TableHead></TableRow></TableHeader><TableBody>
        {queues.map((queue) => {
          const config = (queue.rules ?? {}) as any;
          const conditions = config.conditions ?? {};
          const ruleEntry = Object.entries(conditions)[0];
          return <TableRow key={queue.id}>
            ...
            <TableCell className="text-xs">{ruleEntry ? `${ruleEntry[0]} = ${String(ruleEntry[1])}` : "No rule"}</TableCell>
            <TableCell><Badge variant="outline" className="text-[9px]">{String(config.assignmentType ?? "ROUND_ROBIN")}</Badge></TableCell>
            <TableCell><span className="inline-flex items-center gap-1 text-xs"><Users className="size-3.5" />{Array.isArray(config.userIds) ? config.userIds.length : 0}</span></TableCell>
    ...
    <Dialog open={open} onOpenChange={(next) => { setOpen(next); if (!next) reset(); }}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader><DialogTitle>{editing ? "Edit Queue" : "Create Queue"}</DialogTitle></DialogHeader>
        <div className="space-y-4 py-2">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <div className="space-y-2"><Label>Match field</Label><Select value={field} onValueChange={(v) => setField(v as QueueRule["field"])}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{fields.map((item) => <SelectItem key={item} value={item}>{item}</SelectItem>)}</SelectContent></Select></div>
            <div className="space-y-2"><Label>Operator</Label><Select value={operator} onValueChange={(v) => setOperator(v as QueueRule["operator"])} disabled={field.startsWith("score")}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{operators.map((item) => <SelectItem key={item} value={item}>{item}</SelectItem>)}</SelectContent></Select></div>
          </div>
        ...
        <DialogFooter><Button variant="outline" onClick={() => setOpen(false)}>Cancel</Button><Button onClick={() => void save()}>Save Queue</Button></DialogFooter>
      </DialogContent>
    </Dialog>
  </div>;
}
