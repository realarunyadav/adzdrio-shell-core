            ...
            <SectionCard title="Revenue Growth" className="h-[320px]">
              <div className="flex flex-col items-center justify-center h-full text-muted-foreground">
                ...
                <p className="text-xs opacity-60 italic">'''Do not make any visual modifications. The phrases I write are commands to understand what I want, not to be written down. Understand their content well, then execute what is required.'''
                                            ...
                                            We are deploying the ABOS frontend v2 shell to production at:

https://dashboard.adzdrio.in

The production frontend is served from the repository:

adzdrio-shell-core

The backend is already running separately and must NOT be replaced or modified.

Current production issue:

The live dashboard is loading successfully, but after authentication it is still showing the older dashboard UI/content instead of the intended ABOS Frontend V2 shell.

Please inspect the existing frontend implementation and fix the routing/rendering so that the production application uses the current ABOS Frontend V2 shell as the actual authenticated application shell and dashboard.

Important requirements:

1. Do NOT create a new project.

2. Do NOT replace the existing backend.

3. Do NOT modify backend APIs unless absolutely required for frontend compatibility.

4. Preserve the existing authentication flow.

5. Preserve all existing API integrations and routes.

6. Preserve existing CRM, HRMS, Finance, Leads, Projects, Inventory, AI, Admin and other modules.

7. Reuse the existing components already present in src/components.

8. Make sure src/routes/index.tsx and the authenticated application routing actually render the V2 shell/dashboard rather than an older implementation.

9. Check AppShell, AppHeader, AppSidebar and the authenticated route structure for any legacy dashboard rendering.

10. Do not use mock data where live API data already exists.

11. Keep VITE_API_BASE_URL compatible with production:

    VITE_API_BASE_URL=https://dashboard.adzdrio.in

12. Do not hardcode the Render backend URL into the production frontend.

13. Do not remove existing API functionality.

14. Do not break the existing production backend.

15. Make the smallest safe changes necessary.
...
- existing API service/configuration files
...
- verify that the authenticated route renders the V2 shell

- verify that API requests use the same-origin production API (/api/...)
...
Login → ABOS V2 AppShell → V2 Executive Dashboard → existing live modules/API integrations.</p>
              </div>
            </SectionCard>
