...
function mapLead(lead: any): RapidLead { const customerName = [lead['firstName'], lead['lastName']].filter(Boolean).join(" ").trim() || lead['companyName'] || "Unnamed Lead"; return { ...lead, customerName, customerEmail: lead['email'] ?? "", customerPhone: lead['primaryPhone'], selectedPlanId: lead['selectedPlanId'] ?? "standard", price: Number(lead['price'] ?? lead['expectedValue'] ?? 0), duration: Number(lead['duration'] ?? 0), devices: Array.isArray(lead['devices']) ? lead['devices'] : [], status: String(lead['status'] ?? lead['stage'] ?? "new").toLowerCase() as RapidLead["status"], priority: lead['priority'] ?? "Normal", nextFollowUp: lead['nextFollowUp'] ?? null, confirmationUrl: lead['confirmationUrl'] ?? "", expiresAt: lead['expiresAt'] ?? lead['createdAt'], createdAt: lead['createdAt'] } as RapidLead; }
function normalizeLeadPayload(data: Partial<RapidLead> & Record<string, any>, mode: "create" | "update") { const name = String(data['customerName'] ?? "").trim(); const [firstName, ...rest] = name.split(/\s+/); const payload: Record<string, any> = { ownerId: data['ownerId'], firstName: data['firstName'] ?? (mode === "create" ? firstName || "Lead" : undefined), lastName: data['lastName'] ?? (mode === "create" ? rest.join(" ") || "Contact" : undefined), companyName: data['companyName'], email: data['email'] ?? data['customerEmail'], primaryPhone: data['primaryPhone'] ?? data['customerPhone'], additionalPhones: data['additionalPhones'], city: data['city'], state: data['state'], stage: data['stage'], source: data['source'], sourceUrl: data['sourceUrl'], score: data['score'] === undefined || data['score'] === null ? undefined : Number(data['score']), notes: data['notes'] }; return Object.fromEntries(Object.entries(payload).filter(([, value]) => value !== undefined && value !== "")); }

export const leadsService = {
  getAll: async (params?: Record<string, string | number | undefined>): Promise<RapidLead[]> => { const query = new URLSearchParams(); Object.entries(params ?? {}).forEach(([key, value]) => { if (value !== undefined && value !== "") query.set(key, String(value)); }); const response = await api.get<LeadListResponse>(`/api/leads${query.toString() ? `?${query}` : ""}`); return (response?.items ?? []).map(mapLead); },
  list: async (params?: Record<string, string | number | undefined>) => { const query = new URLSearchParams(); Object.entries(params ?? {}).forEach(([key, value]) => { if (value !== undefined && value !== "") query.set(key, String(value)); }); const response = await api.get<LeadListResponse>(`/api/leads${query.toString() ? `?${query}` : ""}`); return { items: (response?.items ?? []).map(mapLead), pagination: response.pagination }; },
  ...
  getConfirmationDetails: async (token: string) => mapLead(await api.get<any>(`/api/confirmations/${token}`)),
  submitConfirmation: async (token: string, confirmed: boolean, data: any) => api.post(`/api/confirmations/${token}`, { confirmed, ...data }),
};
...
export const dealService = { getAll: (params?: { search?: string; stage?: string; ownerId?: string; accountId?: string }) => { const query = new URLSearchParams(); Object.entries(params ?? {}).forEach(([key, value]) => { if (value) query.set(key, value); }); return api.get<any[]>(`/api/deals${query.toString() ? `?${query}` : ""}`); }, getById: (id: string) => api.get<any>(`/api/deals/${id}`), create: (data: any) => api.post("/api/deals", data), update: (id: string, data: any) => api.patch(`/api/deals/${id}`, data), remove: (id: string) => api.delete(`/api/deals/${id}`), getPipeline: () => api.get<DealPipelineItem[]>("/api/deals/pipeline") };
