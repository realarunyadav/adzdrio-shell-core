...
export function AppSidebar() {
    ...
    const tree = moduleRegistry.navigationTree((permission) => permission ? can(permission) : true);
    ...
    return (<Sidebar collapsible="icon" className="border-sidebar-border">
      ...
      <SidebarContent className="scrollbar-thin">
        {tree.map(({ group, modules }) => (<SidebarGroup key={group.id} className="py-2">
            ...
            <SidebarGroupContent>
              <SidebarMenu>
                ...
                return (<Collapsible key={module.id} asChild defaultOpen={isActiveModule} className="group/collapsible">
                      <SidebarMenuItem>
                        <CollapsibleTrigger asChild>
                          <SidebarMenuButton asChild isActive={isActiveModule} tooltip={module.name}>
                            {planned ? (<Link to="/modules/$moduleId" params={{ moduleId: module.id }} className="flex items-center gap-2">
                                <module.icon className="size-4 shrink-0" aria-hidden/>
                                {!collapsed ? (<span className="flex flex-1 items-center justify-between gap-2">
                              ...
                              </Link>) : (<Link to={module.basePath === "/settings" ? "/settings" : "/"} className="flex items-center gap-2">
                                <module.icon className="size-4 shrink-0" aria-hidden/>
                                {!collapsed ? (<span className="flex flex-1 items-center justify-between gap-2">
                                    <span className="truncate">{module.name}</span>
                        ...
                        {!collapsed && !planned && (<CollapsibleContent>
                            <SidebarMenuSub>
                              <SidebarMenuSubItem>
