...
export function AppHeader() {
    ...
    const initials = (principal?.displayName ?? "AB")
        ...
        .join("")
        .toUpperCase();
    return (<header className="sticky top-0 z-30 flex h-14 items-center gap-3 border-b border-border bg-card px-3 sm:px-4">
      <SidebarTrigger className="text-muted-foreground"/>
      <Separator orientation="vertical" className="hidden h-5 sm:block"/>

      {appConfig.shell.showBreadcrumbs ? (<Breadcrumb className="hidden md:block">
          <BreadcrumbList>
            <BreadcrumbItem>
            ...
            {crumbs.map((crumb) => (<span key={crumb.label} className="flex items-center gap-1.5 sm:gap-2.5">
                <BreadcrumbSeparator />
                <BreadcrumbItem>
                  {crumb.isLast ? (<BreadcrumbPage className="capitalize">{crumb.label}</BreadcrumbPage>) : (<span className="capitalize text-muted-foreground">{crumb.label}</span>)}
                </BreadcrumbItem>
              </span>))}
          </BreadcrumbList>
        </Breadcrumb>) : null}
      ...
      <div className="ml-auto flex items-center gap-2">
        {appConfig.shell.showGlobalSearch ? (<div className="hidden lg:block">
            <SearchBar placeholder="Search ABOS (⌘K)" className="h-9 w-64"/>
          </div>) : null}
        ...
        {allowUserToggle ? (<Button variant="ghost" size="icon" onClick={toggle} aria-label={resolvedMode === "dark" ? "Switch to light theme" : "Switch to dark theme"}>
