...
export function ReconciliationDisplay({ saleId, invoiceId, className }: ReconciliationDisplayProps) {
  const { data: recon, isLoading, error } = useQuery({
    ...
    queryFn: () => financeService.getReconciliation({ saleId, invoiceId }),
  ...
  const currencies = Object.keys(recon.balances);

  return (
    <div className={cn("space-y-4", className)}>
      {currencies.map((currency) => {
        const balance = recon.balances[currency];
        const isSettled = balance.outstanding === 0;
        const isOverpaid = balance.outstanding < 0;

        return (
          <Card key={currency} className="border-border/40 overflow-hidden glass-surface">
            ...
            <CardContent className="p-0">
              <div className="grid grid-cols-2 md:grid-cols-4 divide-x divide-border/40">
                <div className="p-4 space-y-1">
                  ...
                  <p className="text-sm font-black">{currency} {balance.gross.toLocaleString()}</p>
                </div>
                <div className="p-4 space-y-1">
                  <p className="text-[9px] font-bold text-muted-foreground uppercase tracking-tighter">Net Collected</p>
                  <p className="text-sm font-black text-green-600">{currency} {balance.netCollected.toLocaleString()}</p>
                </div>
                <div className="p-4 space-y-1">
                  <p className="text-[9px] font-bold text-muted-foreground uppercase tracking-tighter">Refunded</p>
                  <p className="text-sm font-black text-red-600">{currency} {balance.refunds.toLocaleString()}</p>
                </div>
                <div className="p-4 space-y-1 bg-primary/5">
                  ...
                  <p className={cn("text-sm font-black", balance.outstanding > 0 ? "text-primary" : "text-green-600")}>
                    {currency} {Math.abs(balance.outstanding).toLocaleString()}
                  </p>
                </div>
              </div>
